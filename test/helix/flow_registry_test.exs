defmodule Helix.FlowRegistryTest do
  ...
  describe "flow_info/0" do
    ...
    test "returns flow information for active flows", %{flow_id_1: flow_id} do
      ...
      assert flow_entry.pid == inspect(pid)
      assert flow_entry.alive == true
      assert is_binary(flow_entry.started_at)
    end

    test "provides accurate metadata for multiple flows", %{
      flow_id_1: flow_id_1,
      flow_id_2: flow_id_2
    } do
      {:ok, pid1} = FlowRegistry.start_flow(flow_id_1)
      ...
      assert flow1_info.pid == inspect(pid1)
      assert flow1_info.alive == true
      assert flow2_info.flow_id == flow_id_2
      assert flow2_info.pid == inspect(pid2)
  ...
  describe "process monitoring" do
    test "removes flow from registry when process crashes", %{flow_id_1: flow_id} do
      ...
      # Wait for process cleanup and registry update
      :timer.sleep(200)

      # Ensure process is actually dead
      refute Process.alive?(pid)

      # Registry should eventually be cleaned up by the supervisor
      # In practice, the registry cleanup happens when the process is restarted
      # So we'll test that we can start a new process with the same ID
      {:ok, new_pid} = FlowRegistry.start_flow(flow_id)
      assert new_pid != pid
