defmodule HelixWeb.FlowChannelTest do
  ...
  describe "join flow:* topic" do
    test "joins flow channel successfully", %{flow_id: flow_id, user_id: user_id} do
      ...
      {:ok, reply, socket} =
        subscribe_and_join(socket, FlowChannel, "flow:#{flow_id}", %{"user_id" => user_id})
    ...
    test "receives presence update when user joins", %{flow_id: flow_id, user_id: user_id} do
      ...
      {:ok, _reply, socket1} =
        subscribe_and_join(socket1, FlowChannel, "flow:#{flow_id}", %{"user_id" => user_id})
      ...
      {:ok, _reply, socket2} =
        subscribe_and_join(socket2, FlowChannel, "flow:#{flow_id}", %{"user_id" => user_id_2})

      # Wait for presence broadcast to propagate
      :timer.sleep(50)

      # First user should receive presence update
      assert_push "presence_updated", %{connected_users: connected_users}
      # Both users should be in the connected users list
      assert length(connected_users) == 2
    ...
    test "creates flow when joining non-existent flow" do
      user_id = "test_user_#{:rand.uniform(1000)}"
      {:ok, socket} = connect(UserSocket, %{"user_id" => user_id})

      {:ok, reply, socket} =
        subscribe_and_join(socket, FlowChannel, "flow:nonexistent_flow", %{
          "user_id" => user_id
        })

      # Should create the flow and return its state
      assert reply.id == "nonexistent_flow"
      assert reply.name == "Untitled Flow"
      assert is_list(reply.nodes)
      assert is_list(reply.edges)
      assert user_id in reply.connected_users
  ...
  describe "flow operations" do
    ...
    test "update_viewport does not broadcast (local only)", %{socket: socket} do
      ...
      assert reply.viewport == %{"x" => 200, "y" => 150, "zoom" => 2.0}
  ...
  describe "presence management" do
    test "removes user from presence when they leave", %{flow_id: flow_id, user_id: user_id} do
      ...
      # Leave channel and catch the expected process exit
      Process.flag(:trap_exit, true)
      leave(socket)

      # Give time for cleanup and process any exit messages
      receive do
        {:EXIT, _pid, :shutdown} -> :ok
        {:EXIT, _pid, {:shutdown, :left}} -> :ok
      after
        200 -> :ok
      end
    ...
    test "broadcasts presence changes to remaining users", %{flow_id: flow_id} do
      ...
      {:ok, _reply, socket1} =
        subscribe_and_join(socket1, FlowChannel, "flow:#{flow_id}", %{"user_id" => user_id_1})
  ...
  describe "concurrent operations" do
    test "handles multiple users updating flow", %{flow_id: flow_id} do
      # Create two users
      user1_id = "user_1"
      user2_id = "user_2"

      {:ok, socket1} = connect(UserSocket, %{"user_id" => user1_id})

      {:ok, _reply, socket1} =
        subscribe_and_join(socket1, FlowChannel, "flow:#{flow_id}", %{"user_id" => user1_id})

      {:ok, socket2} = connect(UserSocket, %{"user_id" => user2_id})

      {:ok, _reply, socket2} =
        subscribe_and_join(socket2, FlowChannel, "flow:#{flow_id}", %{"user_id" => user2_id})

      # Both users update metadata
      ref1 = push(socket1, "update_metadata", %{"name" => "User1 Update"})
      assert_reply ref1, :ok, _reply1

      ref2 = push(socket2, "update_metadata", %{"name" => "User2 Update"})
      assert_reply ref2, :ok, _reply2

      # Flow should be in valid state
      {:ok, flow_state} = FlowServer.get_flow(flow_id)
  ...
  describe "integration with FlowServer" do
    test "channel operations are reflected in FlowServer state", %{
      flow_id: flow_id,
      user_id: user_id
    } do
      {:ok, socket} = connect(UserSocket, %{"user_id" => user_id})
