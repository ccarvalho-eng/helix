...
test.describe("Template Functionality", () => {
	...
	test.describe("Template Integration", () => {
		test("should preserve existing nodes when adding template", async ({
			page,
		}) => {
		...
		test("should position template nodes without overlapping existing nodes", async ({
			page,
		}) => {
		...
		test("should create connections between template nodes", async ({
			page,
		}) => {
	...
	test.describe("Template Node Properties", () => {
		test("should load template nodes with correct labels and descriptions", async ({
			page,
		}) => {
	...
	test.describe("Multiple Templates", () => {
		test("should allow adding multiple different templates", async ({
			page,
		}) => {
			const initialNodeCount = await page.locator(".react-flow__node").count();

			// Add first template
			const assassinTemplate = page
				.locator('button:has-text("Assassin")')
				.first();
			if (await assassinTemplate.isVisible()) {
				await assassinTemplate.click();
				await page.waitForTimeout(1000);

				const afterFirstTemplate = await page
					.locator(".react-flow__node")
					.count();
				expect(afterFirstTemplate).toBeGreaterThan(initialNodeCount);

				// Add second template
				const softwareTemplate = page
					.locator('button:has-text("Software")')
					.first();
				if (await softwareTemplate.isVisible()) {
					await softwareTemplate.click();
					await page.waitForTimeout(1000);

					const afterSecondTemplate = await page
						.locator(".react-flow__node")
						.count();
	...
	test.describe("Template Performance", () => {
		test("should load template quickly without blocking UI", async ({
			page,
		}) => {
	...
	test.describe("Template Error Handling", () => {
		test("should gracefully handle template loading errors", async ({
			page,
		}) => {
