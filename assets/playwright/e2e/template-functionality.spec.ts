...
test.describe('Template Functionality', () => {
  ...
  test.describe('Template Loading', () => {
    test('should display template options in dropdown', async ({ page }) => {
      ...
      const templateButton = page.locator('button:has-text("Template"), button:has-text("Add Template"), [data-testid="template-button"], .template-selector');
      ...
      if (await templateButton.first().isVisible()) {
        ...
        const templateOptions = page.locator('[data-testid="template-option"], .template-option, button:has-text("Assassin"), button:has-text("Software"), button:has-text("Customer")');
        const optionCount = await templateOptions.count();
        expect(optionCount).toBeGreaterThan(0);
      } else {
        // If no template button found, look for direct template options
        const directTemplateButtons = page.locator('button:has-text("Assassin"), button:has-text("Software"), button:has-text("Customer")');
    ...
    test('should load Assassins Creed template', async ({ page }) => {
      ...
      const assassinTemplate = page.locator('button:has-text("Assassin"), [data-template="assassins-creed"], .template-assassins-creed');
      ...
      if (await assassinTemplate.first().isVisible()) {
        ...
        const agentNodes = page.locator('.react-flow__node:has-text("Agent"), .react-flow__node:has-text("Assassin")');
    ...
    test('should load Software Automation template', async ({ page }) => {
      ...
      const softwareTemplate = page.locator('button:has-text("Software"), [data-template="software-automation"], .template-software-automation');
      ...
      if (await softwareTemplate.first().isVisible()) {
        ...
        const apiNodes = page.locator('.react-flow__node:has-text("API"), .react-flow__node:has-text("Database"), .react-flow__node:has-text("Transform")');
    ...
    test('should load Customer Service template', async ({ page }) => {
      ...
      const customerTemplate = page.locator('button:has-text("Customer"), [data-template="customer-service"], .template-customer-service');
      ...
      if (await customerTemplate.first().isVisible()) {
        ...
        const serviceNodes = page.locator('.react-flow__node:has-text("Customer"), .react-flow__node:has-text("Support"), .react-flow__node:has-text("Ticket")');
  ...
  test.describe('Multiple Templates', () => {
    ...
    test('should maintain template node uniqueness when adding same template twice', async ({ page }) => {
  ...
  test.describe('Template Error Handling', () => {
    test('should gracefully handle template loading errors', async ({ page }) => {
      ...
      const templateButtons = page.locator('button:has-text("Template"), button:has-text("Assassin"), button:has-text("Software"), button:has-text("Customer")');
