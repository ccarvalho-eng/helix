...
test.describe("Multi-Flow Management System", () => {
	...
	test.describe("Flow Title Management", () => {
		...
		test("should cancel title editing with Escape", async ({ page }) => {
			...
			const originalTitle = await page
				.locator(".flow-builder__flow-title")
				.textContent();
	...
	test.describe("Flow CRUD Operations from Home Page", () => {
		test("should display flows in grid format", async ({ page }) => {
			...
			await expect(page.locator(".home-workflows__count")).toContainText(
				"3 flows",
			);
		});
		...
		test("should show gear menu on hover/click", async ({ page }) => {
			...
			await expect(page.locator(".home-workflow-card__dropdown")).toHaveClass(
				/show/,
			);
			...
			await expect(
				page.locator(
					'.home-workflow-card__dropdown-item:has-text("Duplicate")',
				),
			).toBeVisible();
			await expect(
		...
		test("should rename flow from gear menu", async ({ page }) => {
			...
			await expect(
				page.locator(".home-workflow-card__title-input"),
			).toBeVisible();
		...
		test("should duplicate flow from gear menu", async ({ page }) => {
			...
			await expect(
				page.locator(
					'.home-workflow-card__title:has-text("Original Flow (Copy)")',
				),
			).toBeVisible();
		});
		...
		test("should delete flow with confirmation", async ({ page }) => {
			...
			await expect(page.locator(".home-modal__title")).toContainText(
				"Delete Workflow",
			);
	...
	test.describe("Flow State Persistence", () => {
		test("should preserve flow state when navigating away and back", async ({
			page,
		}) => {
			...
			await page.click(".flow-builder__flow-title");
			await expect(page.locator(".flow-builder__title-input")).toHaveValue(
				"Persistence Test Flow",
			);
		...
		test("should maintain separate state for different flows", async ({
			page,
		}) => {
			// Create first flow
			...
			const flowTitles = await page
				.locator(".home-workflow-card__title")
				.allTextContents();
	...
	test.describe("Flow Statistics", () => {
		test("should display node and connection counts", async ({ page }) => {
			...
			await expect(
				page
					.locator(".home-workflow-card__stat")
