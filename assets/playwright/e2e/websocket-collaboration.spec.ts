...
test.describe("WebSocket and Real-time Collaboration", () => {
	test.describe("WebSocket Connection", () => {
		test("should establish WebSocket connection when entering flow builder", async ({
			page,
		}) => {
	...
	test.describe("Real-time Collaboration Indicators", () => {
		...
		test("should update client count when multiple tabs connect", async ({
			browser,
		}) => {
			...
			try {
				...
				if (
					(await clientCounter1.count()) > 0 &&
					(await clientCounter2.count()) > 0
				) {
	...
	test.describe("Real-time Flow Synchronization", () => {
		...
		test("should sync node property changes between clients", async ({
			browser,
		}) => {
			test.skip(); // Complex multi-client test - skip for now but leave structure
	...
	test.describe("Conflict Resolution", () => {
		...
		test("should preserve data integrity during concurrent operations", async ({
			browser,
		}) => {
	...
	test.describe("Performance and Debouncing", () => {
		test("should maintain good performance with WebSocket enabled", async ({
			page,
		}) => {
