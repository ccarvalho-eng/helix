...
test.describe('WebSocket and Real-time Collaboration', () => {
  test.describe('WebSocket Connection', () => {
    test('should establish WebSocket connection when entering flow builder', async ({ page }) => {
      ...
      const wsConnectionMessages = consoleMessages.filter(msg => 
    ...
    test('should join flow channel for specific flow ID', async ({ page }) => {
      ...
      const channelMessages = consoleMessages.filter(msg => 
        msg.includes('Joined flow channel') || 
        msg.includes('🔌🎯') ||
        msg.includes('flow:')
      );
    ...
    test('should display connection status in UI', async ({ page }) => {
      ...
      const connectionStatus = page.locator('[data-testid="connection-status"], .connection-status, .ws-status, .online-indicator');
  ...
  test.describe('Real-time Collaboration Indicators', () => {
    test('should display client counter', async ({ page }) => {
      ...
      const clientCounter = page.locator('[data-testid="client-count"], .client-count, .user-count, .connected-users');
    ...
    test('should update client count when multiple tabs connect', async ({ browser }) => {
      ...
      try {
        ...
        const clientCounter1 = page1.locator('[data-testid="client-count"], .client-count, .user-count');
        const clientCounter2 = page2.locator('[data-testid="client-count"], .client-count, .user-count');
  ...
  test.describe('Performance and Debouncing', () => {
    test('should debounce frequent changes to avoid spam', async ({ page }) => {
      const consoleMessages: string[] = [];
      page.on('console', msg => {
        if (msg.type() === 'log') {
          consoleMessages.push(msg.text());
      ...
      if (await agentNode.isVisible()) {
        ...
        await page.waitForTimeout(100);
        await agentNode.dragTo(canvas, { targetPosition: { x: 600, y: 200 } });
        
        // Wait for debounce period
        await page.waitForTimeout(2000);
      }

      // Should not send excessive WebSocket messages
      const sendMessages = consoleMessages.filter(msg => 
        msg.includes('flow_change') || msg.includes('📡')
      );
      
      // Should have debounced the messages (not 3 separate sends)
      expect(sendMessages.length).toBeLessThanOrEqual(3);
    });
