defmodule Helix.FlowSessionManager do
  ...
  def handle_call({:join_flow, flow_id, client_id}, _from, state) do
    ...
    {:reply, {:ok, client_count}, %{state | sessions: new_sessions, client_flows: new_client_flows}}
  end
  ...
  def handle_call({:leave_flow, flow_id, client_id}, _from, state) do
    ...
    case Map.get(state.sessions, flow_id) do
      ...
      session ->
        ...
        new_sessions = if client_count == 0 do
          # Remove empty session
          Logger.info("Flow #{flow_id} session ended - no more clients")
          Map.delete(state.sessions, flow_id)
        else
          # Update session
          updated_session = %{session | clients: updated_clients, last_activity: System.system_time(:second)}
          Map.put(state.sessions, flow_id, updated_session)
        end
        
        Logger.info("Client #{client_id} left flow #{flow_id}. Remaining clients: #{client_count}")
  ...
  def handle_call({:get_flow_status, flow_id}, _from, state) do
    case Map.get(state.sessions, flow_id) do
      ...
      session ->
        {:reply, %{active: true, client_count: MapSet.size(session.clients), last_activity: session.last_activity}, state}
    end
  end
  ...
  def handle_call(:get_active_sessions, _from, state) do
    ...
    |> Enum.map(fn {flow_id, session} ->
      {flow_id, %{client_count: MapSet.size(session.clients), last_activity: session.last_activity}}
    end)
    |> Enum.into(%{})
  ...
  def handle_cast({:broadcast_flow_change, flow_id, changes}, state) do
    case Map.get(state.sessions, flow_id) do
      ...
      session ->
        ...
        Logger.debug("Broadcasted flow changes for #{flow_id} to #{MapSet.size(session.clients)} clients")
  ...
  def handle_info(:cleanup_inactive_sessions, state) do
    ...
    {inactive_flows, active_sessions} = state.sessions
    ...
    if length(inactive_flows) > 0 do
      ...
      Logger.info("Cleaned up #{length(inactive_flows)} inactive flow sessions: #{inspect(inactive_flow_ids)}")
    end
    
    # Remove inactive sessions and update client_flows
    new_sessions = Map.new(active_sessions)
    new_client_flows = state.client_flows
